...
def size_distribution_recipe(a_grid, sig_g=20., sig_d=0.2, alpha=1e-4, T=50., v_f=100., rho_s=1.6, ret='sig'):
    ...
    #u_gas = cs * np.sqrt(3. / 2. * alpha) #Birnstiel 2011
    u_gas = cs * np.sqrt(alpha) #Krijt 2016
    #
    ...
    #
    #a_BT = (8. * sig_g / (np.pi * rho_s) * Re**-0.25 * np.sqrt(mu * m_p / (3 * np.pi * alpha)) * (4. * np.pi / 3. * rho_s)**-0.5)**(2. / 5.) # Birnstiel 2011
    a_BT = (3./2.)*(8. * sig_g / (np.pi * rho_s) * Re**-0.25 * np.sqrt(mu * m_p / (3 * np.pi * alpha)) * (4. * np.pi / 3. * rho_s)**-0.5)**(2. / 5.) #Krijt 2016
